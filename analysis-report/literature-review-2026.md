...
### 6.1 异质网络专家发现：完成原计划，且填补"知乎缺口"（首轮实验已完成）

原项目的用户-话题-问题数据天然构成异质信息网络。TUEF [10] 证明"按话题分层的多层图 + 排序"是当前 SOTA，但其评测全部在 Stack Exchange；Roy et al. [7] 确认绝大多数研究单平台且知乎缺席。

**首轮实验已完成**（`expert_finding_analysis.py`，结果见 [expert-finding-results/](expert-finding-results/expert_finding_results.md)）：以 log₁₀(1+赞同数) 为专家性弱标签（仅用于监督与评估、不入特征），在 26,161 用户、313 万内部关注边、1.5 万话题（≥20 用户）、517 万用户-话题边上，同一 60/20/20 划分、同一测试集对比四代方法：
...
三点结论：**(1) 文献预言的代际排序在 2015 知乎快照上完整成立**（异质 GNN > 同质 GNN > 浅层嵌入 > 2015 工具箱），Bing et al. [13] 的论断与 ZhihuRank [1] "内容+结构优于纯链接"的主张均在本数据上得到验证——加入用户-话题信息在嵌入层（+0.006 ρ）和 GNN 层（+0.028 ρ、+4pt P@100）都带来增益；**(2) PageRank——原报告的主力方法——是所有方法中最弱的**；**(3) 一个有趣的错位**：浅层嵌入的全局排序相关性优于工具箱，但其头部识别（NDCG@100、P@100）反而不如 HITS/入度——度类指标恰好在头部专家上最强，而监督式 GNN 在两端同时占优。局限：单一随机划分与种子、未做置信区间；赞同数是全平台累积量，混杂了"受欢迎度"与"专业度"；GNN 为监督方法而工具箱分数为无监督，这一对比正体现"有标签可用时应当用监督图方法"的实践含义。后续可扩展为按话题分组的专家排序（TUEF 式）与 HGB [14] 式的调优基线对照。
...
### 6.6 严格化既有结论：幂律的统计检验（已完成，结论：截断幂律）

原报告的幂律结论基于 log-log 目测，当前网络科学标准要求 Clauset–Shalizi–Newman 式的最大似然拟合 + 备择分布似然比检验。《数据分析与知识发现》2018 年研究 [19] 报告知乎的度分布与行为计数服从**指数截断幂律**而非纯幂律——我们已用 `powerlaw_analysis.py` 对 2015 年原始数据（26,161 用户）完成检验（完整结果见 [powerlaw-results/](powerlaw-results/powerlaw_results.md)）：
...
| answer_num | 2938 | 2.28 | 137 | R=−4.53, p≈5×10⁻¹⁴ | **截断幂律占优（最强证据）** |
| agree_num | 598 | 2.36 | 35086 | R=−2.37, p=0.0002 | **截断幂律占优** |
| thanks_num | 536 | 2.43 | 8819 | R=−2.40, p=0.0002 | **截断幂律占优** |

即：**五项特征中四项拒绝纯幂律、支持指数截断幂律**；补充检验中截断幂律对对数正态也全面持平或占优（follower_num 上显著占优，R=3.30, p<0.001）。这在 2015 年快照上**证实了 2018 年文献的截断幂律论断可回溯至知乎早期**，并修正了原报告"显著幂律"的目测结论：重尾成立，但尾部存在指数截断——唯一例外是关注数（出度，用户主动行为），与"被动获得的注意力/声誉指标（粉丝、赞同、感谢）存在饱和上限，主动关注行为不受限"的解释一致。方法脚本经合成数据双向验证（纯幂律数据不误报、截断数据以 |R|>12 正确识别）。
