...
- 《社会化问答社区用户行为统计特性及其动力学分析：以知乎网为例》（**《数据分析与知识发现》** 2018, 2(4): 48–58）[19] 用爬取的知乎公开数据发现：用户活动的事件间隔与等待时间服从幂指数分别为 0.68 与 1.51 的幂律分布（阵发性、非泊松）；该外部研究报告**关注网络度分布及回答/赞同/评论数服从指数截断幂律**。这与原项目的 log-log 目测结论形成对照，但本项目自己的模型归类仍须等待 canonical GOF 与直接备择比较重跑，不能借外部样本代替。
...
### 6.1 异质网络专家发现：完成原计划，且填补"知乎缺口"（先导实验已完成）

原项目的用户-话题-问题数据天然构成异质信息网络。TUEF [10] 证明"按话题分层的多层图 + 排序"是当前 SOTA，但其评测全部在 Stack Exchange；Roy et al. [7] 确认绝大多数研究单平台且知乎缺席。

**先导实验已完成**（`expert_finding_analysis.py`，结果见 [expert-finding-results/](expert-finding-results/expert_finding_results.md)）：以 log₁₀(1+赞同数) 为专家性弱标签（仅用于监督与评估、不入特征），在 26,161 用户、313 万内部关注边、1.5 万话题（≥20 用户）、517 万用户-话题边上，用一次固定 60/20/20 划分和同一测试集对比四代方法：

| 方法（代际） | Spearman ρ | NDCG@100 | P@100 |
|---|---|---|---|
| PageRank（2015 工具箱） | 0.528 | 0.679 | 0.40 |
| in-degree / HITS authority（2015 工具箱） | 0.649 | 0.781 / 0.817 | 0.50 / 0.53 |
| DeepWalk + ridge（一代嵌入） | 0.745 | 0.648 | 0.36 |
| Metapath（用户-话题）walks + ridge（异质嵌入） | 0.751 | 0.659 | 0.40 |
| GraphSAGE（同质 GNN） | 0.843 | 0.820 | 0.56 |
| **Hetero-GraphSAGE（异质 GNN，关注图+用户-话题）** | **0.871** | **0.845** | **0.60** |

这次固定划分呈现三点先导信号：**(1) 方法排序为异质 GNN > 同质 GNN > 浅层嵌入 > 2015 工具箱**，加入用户-话题信息在嵌入层（+0.006 ρ）和 GNN 层（+0.028 ρ、+4pt P@100）都有增益；**(2) PageRank 在这一次测试集上最弱**；**(3) 浅层嵌入的全局排序相关性高于工具箱，但头部识别反而不如 HITS/入度。以上是待重复实验验证的假设，不是已经稳定成立的代际定律。局限包括：单一随机划分与种子、未做置信区间；赞同数混杂"受欢迎度"与"专业度"；GNN 为监督方法而工具箱分数为无监督。后续需做多种子、匹配的监督基线、按话题分组排序与 HGB [14] 式调优对照。
...
### 6.6 严格化既有结论：幂律的统计检验（基础拟合已完成，canonical 模型选择待重跑）

原报告的幂律结论基于 log-log 目测，当前网络科学标准要求 Clauset–Shalizi–Newman 式的最大似然拟合、bootstrap GOF 与备择分布似然比检验。《数据分析与知识发现》2018 年研究 [19] 报告知乎的度分布与行为计数服从**指数截断幂律**而非纯幂律。两条 Claude 研究线已在 2015 年原始数据（26,161 用户）上得到相同的 MLE 和纯幂律对备择分布结果；canonical 流水线采用 `analysis/powerlaw_fit.py`，并额外要求直接比较截断幂律与对数正态。下表是 profile-only 归档中的既有纯幂律 vs 截断幂律结果：

| 特征 | n_tail | α | xmin | 纯幂律 vs 截断幂律 | 结论 |
|---|---|---|---|---|---|
| followee_num | 1587 | 2.63 | 550 | R=−0.96, p=0.079 | 纯幂律尚可成立（不可区分） |
| follower_num | 268 | 2.58 | 75863 | R=−1.95, p=0.010 | **截断幂律占优** |
| answer_num | 2938 | 2.28 | 137 | R=−4.53, p≈5×10⁻¹⁴ | 截断幂律优于纯幂律；尚未据此排除对数正态 |
| agree_num | 598 | 2.36 | 35086 | R=−2.37, p=0.0002 | 截断幂律优于纯幂律；尚未据此排除对数正态 |
| thanks_num | 536 | 2.43 | 8819 | R=−2.40, p=0.0002 | 截断幂律优于纯幂律；尚未据此排除对数正态 |

合并后的七序列结果中，六项在 bootstrap GOF 下拒绝纯幂律；关注数的 GOF 仍与纯幂律相容。对 answer、agree、thanks 和两个诱导度序列，截断幂律与对数正态在各自对纯幂律的比较中都占优，因此不能靠检查顺序命名最终模型。原报告"五项均显著幂律"的目测结论已被修正，但"究竟是截断幂律还是对数正态"必须等 canonical 直接比较重跑后再定稿。此前未提交原始表的补充比较数字不再作为 canonical 证据。
